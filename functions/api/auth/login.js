...
export async function onRequestPost({ request, env }) {
  ...
  const user = await env.DB.prepare(
    'SELECT id, name, username, email, phone, lang, password_hash, salt FROM users WHERE username = ?'
  ...
  const token = await signJWT(
    { sub: user.id, username: user.username, name: user.name, email: user.email || '', phone: user.phone || null, lang: userLang, exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 30 },
    env.JWT_SECRET
  )
